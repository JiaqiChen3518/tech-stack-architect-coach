...
- 本范例共 **6 条留痕**（M0 三条 + M1/M2 各一条 + M4 一条）、**14 条术语**（M0-M3 八条 + M4 六条），符合"4-6 条留痕、10-14 条术语"的颗粒度参考；
- **冷启动读取范围示例**（本范例对应第 3 窗口续接）：教练读【已讲术语表】全部 14 条 +【执行留痕】最近 2 个模块（M2、M4）共 2 条；M0 的 3 条留痕不默认读取，但其中"redis.conf 持久化"一条若涉及环境判断可按需翻看；
